...
const features = [
  {
    title: "会追问的面试官",
    desc: "不是念题式提问。Mira 会顺着你的回答深化细节、追问权衡取舍，像真实面试一样带来压力与节奏。",
    icon: <span className="block h-4 w-4 rounded-full border-[3px] border-orange-500" />,
  },
  {
    title: "基于你的简历定制",
    desc: "上传简历后，题目会围绕你的真实项目、技术栈与目标岗位生成，问到简历里的关键细节。",
    icon: <span className="block h-4 w-4 rounded-[3px] bg-orange-500" />,
  },
  {
    title: "结构化评估报告",
    desc: "每道题都有你的回答、参考答案、耗时与改进建议，最后给出综合评级，清楚知道下一步该练什么。",
    icon: <span className="block h-4 w-4 rotate-45 border-[3px] border-orange-500" />,
  },
];
...
export default function LandingPage() {
  return (
    <div className="relative overflow-hidden">
      ...
      <section className="mx-auto grid max-w-[1180px] grid-cols-1 items-center gap-14 px-6 pt-28 pb-10 md:grid-cols-[1.05fr_.95fr] md:px-10 md:pt-32">
        <div className="animate-mira-page-in">
          ...
          <p className="m-0 mb-[34px] max-w-[540px] text-lg leading-relaxed text-[#525252]">
            上传你的简历，Mira 会像真正的面试官一样，围绕经历与目标岗位展开一轮完整、有深度、会追问的仿真面试，并给出结构化评估报告。
          </p>
          <div className="flex flex-wrap items-center gap-3.5">
            <Link
              href="/auth"
              transitionTypes={["nav-forward"]}
              className="mira-button rounded-[11px] bg-orange-500 px-[26px] py-3.5 text-[15px] font-medium text-white shadow-[0_6px_20px_rgba(249,115,22,0.28)] hover:text-white hover:shadow-[0_10px_26px_rgba(249,115,22,0.36)]"
            >
              开始一场面试 →
            </Link>
            <Link
              href="/interview/demo"
              transitionTypes={["nav-forward"]}
              className="mira-button rounded-[11px] border border-[#e5e5e5] bg-white px-6 py-3.5 text-[15px] font-medium text-[#0a0a0a] hover:border-[#d4d4d4] hover:bg-[#fafafa] hover:text-[#0a0a0a]"
            >
              观看演示
            </Link>
          </div>
          <div className="mt-[34px] flex items-center gap-4 text-[13px] text-[#a3a3a3]">
            ...
            已帮助 <span className="font-medium text-[#0a0a0a]">12,800+</span> 位候选人准备面试
          </div>
        </div>

        <div className="relative animate-mira-soft-pop [animation-delay:.14s]">
          <div
          ...
          <div className="animate-mira-float relative rounded-[22px] border border-[#ededed] bg-white p-[22px] shadow-[0_24px_60px_-20px_rgba(0,0,0,0.18)]">
            ...
            <div className="mb-3.5 flex gap-2.5">
              ...
              <div className="rounded-[4px_14px_14px_14px] bg-[#f6f6f6] px-3.5 py-[11px] text-[13.5px] leading-[1.55] text-[#171717]">
                你在项目里提到用了状态管理，能说说当时为什么不直接用 Context，而选择额外的库吗？
              </div>
            </div>
            <div className="mb-3.5 flex flex-row-reverse gap-2.5">
              ...
              <div className="rounded-[14px_4px_14px_14px] border border-[#ffe0cc] bg-[#fff5ee] px-3.5 py-[11px] text-[13.5px] leading-[1.55] text-[#7c2d12]">
                主要是因为跨层级的高频更新，Context 会引起较大范围重渲染……
              </div>
            </div>
      ...
      <section className="mx-auto mt-10 max-w-[1180px] px-6 md:px-10">
        ...
        <div className="mira-stagger flex flex-wrap justify-center gap-2.5">
          {trackTags.map((tag) => (
            <span key={tag} className="mira-surface rounded-full border border-[#eee] px-4 py-2 text-[13.5px] text-[#525252]">
              {tag}
            </span>
          ))}
        </div>
      ...
      <section id="features" className="mx-auto mt-[110px] max-w-[1180px] scroll-mt-[90px] px-6 md:px-10">
        ...
        <div className="mira-stagger grid grid-cols-1 gap-5 md:grid-cols-3">
          {features.map((f) => (
            <div key={f.title} className="mira-surface rounded-[18px] border border-[#eee] p-7 hover:border-[#ffd9bd]">
              ...
              <p className="m-0 text-sm leading-relaxed text-[#737373]">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>

      <section id="how" className="mx-auto mt-[110px] max-w-[1180px] scroll-mt-[90px] px-6 md:px-10">
        <div className="mb-11 flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/auth" transitionTypes={["nav-forward"]} className="mira-button rounded-[10px] border border-[#e5e5e5] bg-white px-[22px] py-3 text-sm font-medium text-[#0a0a0a] hover:text-[#0a0a0a]">
            立即开始
          </Link>
        </div>
        <div className="mira-stagger grid grid-cols-2 gap-[18px] md:grid-cols-4">
          {steps.map((s, i) => (
            <div key={s.n} className={`pt-5 ${i === 0 ? "border-t-2 border-[#0a0a0a]" : "border-t-2 border-[#e5e5e5]"}`}>
              <div className="mb-3.5 font-display text-sm text-[#a3a3a3]">{s.n}</div>
              <h3 className="m-0 mb-2 text-base font-semibold">{s.title}</h3>
              <p className="m-0 text-[13.5px] leading-relaxed text-[#737373]">{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <footer id="faq" className="mx-auto mt-20 flex max-w-[1180px] scroll-mt-[90px] flex-wrap items-center justify-between gap-4 border-t border-[#f0f0f0] px-6 pt-10 pb-[60px] md:px-10">
        <Logo size="sm" />
        <div className="text-[13px] text-[#a3a3a3]">
          © 2026 MiraPrep Labs · 让每一次面试都有准备
        </div>
      </footer>
    </div>
  );
