    ...
    <div data-session={sessionId} className="min-h-screen bg-surface-subtle">
      <div className="sticky top-0 z-20 flex items-center justify-between border-b border-border-subtle bg-surface/92 px-6 py-5 backdrop-blur-[12px] md:px-7">
        <div className="flex items-center gap-4">
          <Logo />
          <Link href="/dashboard" transitionTypes={["nav-back"]} className="mira-button rounded-[9px] border border-border bg-surface px-3.5 py-2 text-[13px] text-muted-foreground">
            ← 工作台
          </Link>
        </div>
        <div className="flex items-center gap-2.5">
          <ThemeToggle />
          <button className="mira-button rounded-[9px] border border-border bg-surface px-4 py-2 text-[13px]">
            导出 PDF
          </button>
      ...
      <div className="animate-mira-page-in mx-auto max-w-[840px] px-6 pt-10 pb-20 md:px-8">
        <div className="mb-2 font-display text-[13px] text-muted-foreground">
        ...
        <div className="mb-4 grid grid-cols-1 gap-8 rounded-[20px] border border-border-subtle bg-surface p-7 sm:grid-cols-[auto_1fr] sm:items-center">
          <div className="text-center sm:border-r sm:border-muted sm:pr-8">
            <div className="font-display text-[60px] leading-none font-bold text-orange-500">A-</div>
            <div className="mt-1.5 text-[12.5px] text-muted-foreground">综合评级 · 超过 78% 候选人</div>
          </div>
          <div className="flex flex-col gap-3.5">
            {dimensions.map((d) => (
              <div key={d.label}>
                ...
                <div className="h-[7px] overflow-hidden rounded-sm bg-muted">
                  <div className={`animate-mira-progress h-full rounded-sm ${d.active ? "bg-primary" : "bg-border"}`} style={{ width: `${d.pct}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mb-9 rounded-2xl border border-primary/20 bg-primary-soft px-6 py-5">
          <div className="mb-2 text-[13px] font-semibold text-primary">总体评语</div>
        ...
        <div className="mb-[18px] flex items-center justify-between">
          ...
          <span className="text-[13px] text-muted-foreground">共 {all.length} 题</span>
        </div>
        ...
        <div className="flex flex-col gap-3.5">
          {visible.map((item) => {
            ...
            return (
            <div key={item.n} className="mira-surface animate-mira-soft-pop overflow-hidden rounded-[18px] border border-border-subtle bg-surface">
              <div className="flex items-center gap-3.5 border-b border-muted px-[22px] py-[18px]">
                ...
                <div className="shrink-0 text-right">
                  {viewedHint && (
                    <div className="mb-1 rounded-full bg-primary-soft px-2.5 py-1 text-[11px] font-medium text-primary">
                      查看过提示
                    </div>
                  )}
                  ...
                  <div className="text-[11.5px] text-muted-foreground">用时 {item.time}</div>
                </div>
              </div>
              <div className="grid grid-cols-1 gap-5 px-[22px] py-[18px] sm:grid-cols-2">
                <div>
                  <div className="mb-2 text-xs font-medium text-muted-foreground">你的回答</div>
                  <p className="m-0 text-[13.5px] leading-relaxed text-foreground">{item.a}</p>
                </div>
                <div>
                  <div className="mb-2 text-xs font-medium text-orange-500">参考答案要点</div>
                  <p className="m-0 text-[13.5px] leading-relaxed text-foreground">{item.reference}</p>
                </div>
              </div>
              <div className="border-t border-muted bg-surface-subtle px-[22px] py-3.5 text-[13px] leading-relaxed text-muted-foreground">
                <b className="text-foreground">建议：</b>
                {item.tip}
              </div>
            </div>
            );
          ...
          {hiddenCount > 0 && (
            <div onClick={() => setExpanded(true)} className="mira-button cursor-pointer p-2 text-center">
              <span className="text-[13.5px] text-primary">展开其余 {hiddenCount} 题 →</span>
            </div>
          )}
          {expanded && (
            <div onClick={() => setExpanded(false)} className="mira-button cursor-pointer p-2 text-center">
              <span className="text-[13.5px] text-muted-foreground">收起 ↑</span>
            </div>
          )}
        </div>
      </div>
