...
MiraPrep 是一款 AI 模拟面试产品。用户上传简历、选择目标岗位后，AI 面试官会像真正的面试官一样，围绕简历里的真实项目与技术栈展开一轮完整、有深度、会**动态追问**的仿真面试。文字与语音两种面试模式均已实现（语音走 WebSocket 流式 ASR/TTS，需配置 Deepgram 与 OpenAI 密钥）。面试结束后，系统生成**综合评级 + 逐题分析报告**（你的回答、参考答案、耗时、改进建议），帮助求职者发现短板、迭代表现。
...
| 登录 / 注册 | 邮箱密码认证 + Google / GitHub 登录（配对应 client id 后启用） | 已接后端 |
...
| 批改与报告 | AI 批改、报告持久化、统计聚合 | 已实现 |
| 语音面试 | ASR/TTS、录音与播报 | 已实现；需配置 ASR/TTS 提供商密钥 |
...
| `/` | 落地页 | ✅ |
| `/auth` | 登录 / 注册 | ✅ |
| `/auth/callback/github` | GitHub 授权回调（换登录态后立即跳走） | ✅ |
| `/onboarding` | 初次引导 | ✅ |
| `/dashboard` | 个人工作台 | ✅ |
...
| **M1 基础闭环** | 认证、简历解析、配置向导、纯文字面试、批改、报告与统计 | ✅ 已完成并上线 |
| **M2 语音与体验** | 语音 ASR/TTS、面试官动画、评级揭晓动效、能力雷达图 | ✅ 已完成（T-112~T-116） |
