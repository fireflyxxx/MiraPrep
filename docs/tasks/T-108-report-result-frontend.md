...
- 评级页和报告页先轮询 T-106 `GET /reports/{sessionId}/status`；`grading` 使用稳定等待态并持续
  等待，`ready` 后才请求报告详情，`failed` 显示明确失败态。禁止把详情 404 当作生成中，
  也禁止用固定重试次数给真实批改耗时设置隐含上限。
...
7. 报告生成耗时超过 3 分钟时仍保持可理解的等待态；生成中不请求报告详情、不产生 404。
