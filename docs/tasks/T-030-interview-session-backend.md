...
| 轨道 | 里程碑 | 预估 | 依赖 | 阻塞 |
|---|---|---|---|---|
| Backend-Spring | M1 | 1.5d | T-004, T-003 | T-032, T-041, T-061 |
...
- `POST /interviews/{id}/end` body `{reason:"manual"|"timeout"|"completed"}` → 置 `status=completed|aborted`、`ended_at`、`grading_status=pending`，触发 FastAPI 批改（T-050，接口先留调用点）。
- `GET /interviews?page&size&status?` → 分页：
  ...
  本任务先冻结并返回这些字段；T-051 落地报告后补全 `grade` 与 `reportStatus` 的真实投影，T-030 阶段允许 `grade=null`、`reportStatus=none`。
