...
- LLM 结构化：把抽取文本作为**不可信数据**喂给 LLM，输出固定 schema：
  ...
  ```
  结构化抽取用 LangChain chain：`ChatPromptTemplate | ChatAnthropic.with_structured_output(ResumeParsed)`（`ResumeParsed` 为上述 schema 的 Pydantic 模型），不手写 JSON 解析；解析失败/字段缺失允许部分填充。
