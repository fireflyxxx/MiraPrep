# T-107 · 工作台首页联调（问候 + 统计 + 简历模块组合）

| 轨道 | 里程碑 | 预估 | 依赖 | 阻塞 |
|---|---|---|---|---|
| Frontend | M1 | 1d | T-005, T-006, T-011, T-022, T-106 | T-108, T-109 |
...
- **不做**：不在工作台展示面试记录、综合评级、五维雷达或筛选；这些属于 T-109/T-108。简历上传/CRUD 行为由 T-022 负责。本月额度没有真实契约时不接假数据。

## 技术规格
- 新增 `src/lib/api/stats.ts`，冻结 `StatsOverview` 类型并提供 `useOverviewStats()`；T-108/T-109 后续复用，禁止各自重复声明统计响应。
