...
## 执行顺序（2026-07-24 状态同步）

T-001~T-040 与 T-101~T-103 已完成并进入版本历史；T-104 已在当前分支形成提交。
T-105/T-106 已实现，且“运行时结束 → Spring 组装持久化事实 → FastAPI 批改队列 →
Spring 报告回调”已经通过两端自动化测试；真实 LLM、MySQL 和 Redis 的跨进程 HTTP
验收及提交发布仍待收口。后续继续按编号执行，**做到 T-111 即上线** ★，T-112 起为
上线后的 M2/M3 迭代。

节奏与例外：

- **一个任务 = 一个 PR**，任务内按可运行的逻辑步小步提交；PR 描述按任务文档「验证方式」贴证据。
- T-102 已建立三服务 CI；后续所有 PR 复用相同 lint/test/build/coverage 基线。
- 当前发布收口顺序：T-104 → T-105 → T-106；完成后从 T-107 继续。
- 可并行（多 agent 时）：T-108 ‖ T-109、T-112 ‖ T-113。
- T-104 发布收口时仍需保留一条端到端文字面试手工证据，单元测试不能替代真实链路验收。
...
派单时在此维护状态（TODO / DOING / REVIEW / DONE）。`REVIEW` 表示实现与本地验证已完成，但尚未完成提交、推送或合并收口：
...
| T-102 | DONE | 三服务测试基线、覆盖率与 GitHub Actions CI 已提交 |
| T-103 | DONE | 消息持久化、增量读取、幂等与会话生命周期已合并 |
| T-104 | REVIEW | 文字面试 SSE、重连、刷新恢复与真实回看已在当前分支提交，待发布收口 |
| T-105 | REVIEW | 批改 chain、确定性聚合、Redis 可靠队列及运行时到 Spring 的触发链已实现；FastAPI 162 tests 通过，待真实环境验收/发布 |
| T-106 | REVIEW | 请求组装、报告回调/查询、统计聚合与列表投影已实现；Spring 75 tests 通过，待真实环境验收/发布 |
| T-107~T-111 | DONE | 工作台/评级页/我的面试、安全加固与生产部署已合并 |
| T-112 | DONE | WS 流式 ASR 已跨进程验收（桩 provider，最终转写 47ms）；真实 Deepgram key 待复测 |
| T-113 | DONE | 句级流式 TTS 已跨进程验收（首帧 422ms、2 句分段）；真实 OpenAI key 待复测 |
| T-114 | DONE | 语音模式已验收；配置向导语音开关此前被丢弃，已修复。声纹波形需一次带麦克风的人工走查 |
| T-115 | DONE | 动效体系已验收；修复 CountUp 时钟混用与后台标签页显示 0 的问题 |
| T-116 | DONE | 深色面试页、揭晓动效、雷达图已在真实浏览器验收 |
| T-117~T-123 | TODO | 从 T-117 继续 |
